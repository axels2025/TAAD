...
def daemon_install_service(
    ...
    dashboard: bool = typer.Option(False, "--dashboard", help="Also install dashboard plist"),
    dashboard_only: bool = typer.Option(False, "--dashboard-only", help="Only install dashboard plist"),
) -> None:
    """Install launchd services for daemon, watchdog, and/or dashboard.

    Creates plist files in ~/Library/LaunchAgents/ and loads them
    with launchctl. The daemon plist restarts on crash only;
    the watchdog plist stays running always; the dashboard waits
    for the daemon PID file before starting.
    """
    ...
    install_daemon = not watchdog_only and not dashboard_only
    install_watchdog = not daemon_only and not dashboard_only
    install_dashboard = dashboard or dashboard_only
    ...
    if install_dashboard:
        # Create wrapper script that waits for daemon before starting dashboard
        wrapper_path = project_root / "scripts" / "start-dashboard.sh"
        _create_dashboard_wrapper(wrapper_path, project_root, venv_python)

        plist_path = launch_dir / "com.taad.dashboard.plist"
        xml = _generate_plist(
            label="com.taad.dashboard",
            program_args=[str(wrapper_path)],
            working_directory=str(project_root),
        ...
        plist_path.write_text(xml)
        subprocess.run(["launchctl", "load", str(plist_path)], check=True)
        installed.append(("com.taad.dashboard", plist_path))
...
def daemon_uninstall_service() -> None:
    ...
    for label in ("com.taad.daemon", "com.taad.watchdog", "com.taad.dashboard"):
...
def _create_dashboard_wrapper(
    wrapper_path: Path,
    project_root: Path,
    venv_python: Path,
) -> None:
    """Create a shell script that waits for the daemon PID file, then starts the dashboard."""
    pid_file = project_root / "run" / "taad.pid"
...
cd "{project_root}"
exec "{venv_python}" -m src.cli.main dashboard
""")
...
def _generate_plist(
    ...
    Args:
        ...
        run_at_load: Start the service immediately when loaded (e.g. at login)
